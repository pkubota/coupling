...
!use ESMF!,  only: ESMF_TimeLE, ESMF_TimeGE
...
use NUOPC,       only: NUOPC_GetTimestamp
...
#ifndef CESMCOUPLED
!PK  use shr_is_restart_fh_mod, only : init_is_restart_fh, is_restart_fh, is_restart_fh_type
#endif
...
!PKtype(is_restart_fh_type) :: restartfh_info     ! For flexible restarts in UFS
#endif
...
subroutine SetServices(gcomp, rc)
  ...
  if (ChkErr(rc,__LINE__,u_FILE_u)) return
  ! Sobrescreve CheckImport com rotina vazia.
  ! O NUOPC_ModelBase (linha 2370) aborta se campos do importState nao
  ! tiverem timestamp == currTime. Desabilitamos aqui pois o DATM seta
  ! corretamente mas o NUOPC 8.9.0 rejeita mesmo assim.
  ...
  call NUOPC_CompSpecialize(gcomp, specLabel=model_label_CheckImport, &
    specRoutine=CheckImportSkip, rc=rc)
  if (ChkErr(rc,__LINE__,u_FILE_u)) return
...
!! to use.
!!
!! For this MOM cap, we are using IPDv03 (phases IPDv03p1 and IPDv03p3).
!!
!! @param gcomp an ESMF_GridComp object
...
subroutine InitializeRealize(gcomp, importState, exportState, clock, rc)
  ...
  if (geomtype == ESMF_GEOMTYPE_MESH) then
    ...
    call MOM_RealizeFields(exportState, fldsFrOcn_num, fldsFrOcn, "Ocn export", &
         grid=gridOut, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
  endif
...
subroutine ModelAdvance(gcomp, rc)
  ...
  integer              :: nan_count
  character(len=556) :: msg
  ! Campos que voc? quer inspecionar
  character(len=64), parameter :: checkFields(8) = [ &
  "Faxa_rain       ", &
  "Faxa_snow       ", &
  "Foxx_lwnet      ", &
  "Foxx_swnet_vdr  ", &
 ...
 ! - DIAGN?STICO DE IMPORT FIELDS ------------------------------
  do i = 1, size(checkFields)
    fieldName = trim(adjustl(checkFields(i)))

    call ESMF_StateGet(importState, itemName=fieldName, &
                       field=field, rc=localrc)
    if (localrc /= ESMF_SUCCESS) then
      write(msg,'(A,A)') &
      '[MOM_CAP DIAG] Campo NAO encontrado no importState: ', trim(fieldName)
    ...
    if (localrc /= ESMF_SUCCESS) then
       write(msg,'(A,A)') &
      '[MOM_CAP DIAG] Erro ao acessar farrayPtr: ', trim(fieldName)
      call ESMF_LogWrite(trim(msg), ESMF_LOGMSG_INFO, rc=rc)
      cycle
    end if

    fmin      = minval(farrayPtr)
    fmax      = maxval(farrayPtr)
    nan_count = count(farrayPtr /= farrayPtr)   ! NaN check idiom Fortran
    write(msg,'(A,A20, A,E14.6,  A,E14.6, A,I8)') &
      '[MOM_CAP DIAG] ', trim(fieldName), &
      '  min=', fmin, '  max=', fmax, &
      '  NaNs=', nan_count      
    call ESMF_LogWrite(trim(msg), ESMF_LOGMSG_INFO, rc=rc)
  end do
  ! --------------------------------------------------------------
    ...
    ! Zera Ice_ocean_boundary antes de popular via importState.
    ! Evita NaN no Step=0 quando use_coldstart=.true. (ocean lag desativado)
    ! e o DATM ainda nao entregou campos validos para todos os ponteiros.
    ! Campos mapeados a partir do DATM (Datm_cap.F90):
    !   Foxx_taux/tauy -> u_flux/v_flux
    !   Foxx_sen       -> t_flux
    !   Foxx_evap      -> q_flux
    !   Foxx_lwnet     -> lw_flux
    !   Foxx_swnet_*   -> sw_flux_vis_dir/dif/nir_dir/nir_dif
    !   Faxa_rain      -> lprec
    !   Faxa_snow      -> fprec
    !   Sa_pslv        -> p
    if (currTime == startTime) then
      call ESMF_LogWrite( &
        "MOM_cap: zerando Ice_ocean_boundary no Step=0 (cold-start guard)", &
        ESMF_LOGMSG_INFO)
      if (associated(Ice_ocean_boundary%u_flux))          Ice_ocean_boundary%u_flux          = 0.0
      if (associated(Ice_ocean_boundary%v_flux))          Ice_ocean_boundary%v_flux          = 0.0
      if (associated(Ice_ocean_boundary%t_flux))          Ice_ocean_boundary%t_flux          = 0.0
      if (associated(Ice_ocean_boundary%q_flux))          Ice_ocean_boundary%q_flux          = 0.0
      if (associated(Ice_ocean_boundary%salt_flux))       Ice_ocean_boundary%salt_flux       = 0.0
      if (associated(Ice_ocean_boundary%lw_flux))         Ice_ocean_boundary%lw_flux         = 0.0
      if (associated(Ice_ocean_boundary%sw_flux_vis_dir)) Ice_ocean_boundary%sw_flux_vis_dir = 0.0
      if (associated(Ice_ocean_boundary%sw_flux_vis_dif)) Ice_ocean_boundary%sw_flux_vis_dif = 0.0
      if (associated(Ice_ocean_boundary%sw_flux_nir_dir)) Ice_ocean_boundary%sw_flux_nir_dir = 0.0
      if (associated(Ice_ocean_boundary%sw_flux_nir_dif)) Ice_ocean_boundary%sw_flux_nir_dif = 0.0
      if (associated(Ice_ocean_boundary%lprec))           Ice_ocean_boundary%lprec           = 0.0
      if (associated(Ice_ocean_boundary%fprec))           Ice_ocean_boundary%fprec           = 0.0
      if (associated(Ice_ocean_boundary%lrunoff))         Ice_ocean_boundary%lrunoff         = 0.0
      if (associated(Ice_ocean_boundary%frunoff))         Ice_ocean_boundary%frunoff         = 0.0
      if (associated(Ice_ocean_boundary%p))               Ice_ocean_boundary%p               = 0.0
      if (associated(Ice_ocean_boundary%seaice_melt_heat))Ice_ocean_boundary%seaice_melt_heat= 0.0
      if (associated(Ice_ocean_boundary%seaice_melt))     Ice_ocean_boundary%seaice_melt     = 0.0
      if (associated(Ice_ocean_boundary%hrofl))           Ice_ocean_boundary%hrofl           = 0.0
      if (associated(Ice_ocean_boundary%hrofi))           Ice_ocean_boundary%hrofi           = 0.0
      if (associated(Ice_ocean_boundary%hrain))           Ice_ocean_boundary%hrain           = 0.0
      if (associated(Ice_ocean_boundary%hsnow))           Ice_ocean_boundary%hsnow           = 0.0
    end if
...
#ifndef CESMCOUPLED
!PK    call is_restart_fh(clock, restartfh_info, write_restartfh)
...
subroutine ModelSetRunClock(gcomp, rc)
  ...
  ! DEPOIS ? busca o stopTime diretamente do driver clock:
  type(ESMF_Clock) :: driverClock
  type(ESMF_Time)  :: realStopTime
  ...
  if (first_time) then
    ...
    if (cesm_coupled) then
      ...
      if (ChkErr(rc,__LINE__,u_FILE_u)) return
!PK      call init_is_restart_fh(mcurrTime, dt_cpl, is_root_pe(), restartfh_info)
#endif
    endif
    ...
    ! Cria stop_alarm usando stop_ymd e stop_tod passados pelo esm.F90.
    ! O driverClock do componente filho NAO tem o stopTime global --
    ! o NUOPC seta stopTime = currTime+timeStep a cada RunPhase.
    ! O esm.F90 passa stop_ymd (YYYYMMDD) e stop_tod (segundos do dia)
    ! como atributos NUOPC para que o stop_alarm seja criado corretamente.
    block
      integer           :: stop_ymd_i, stop_tod_i
      integer           :: syy, smm, sdd, ss_local
      type(ESMF_Time)   :: realStopTime_local

      stop_ymd_i = 0 ; stop_tod_i = 0

      call NUOPC_CompAttributeGet(gcomp, name="stop_ymd", value=cvalue, &
           isPresent=isPresent, isSet=isSet, rc=rc)
      if (ChkErr(rc,__LINE__,u_FILE_u)) return
      if (isPresent .and. isSet) read(cvalue,*) stop_ymd_i

      call NUOPC_CompAttributeGet(gcomp, name="stop_tod", value=cvalue, &
           isPresent=isPresent, isSet=isSet, rc=rc)
      if (ChkErr(rc,__LINE__,u_FILE_u)) return
      if (isPresent .and. isSet) read(cvalue,*) stop_tod_i

      if (stop_ymd_i > 0) then
        syy      = stop_ymd_i / 10000
        smm      = mod(stop_ymd_i / 100, 100)
        sdd      = mod(stop_ymd_i, 100)
        ss_local = stop_tod_i
        call ESMF_TimeSet(realStopTime_local, yy=syy, mm=smm, dd=sdd, &
             s=ss_local, calkindflag=ESMF_CALKIND_GREGORIAN, rc=rc)
        if (ChkErr(rc,__LINE__,u_FILE_u)) return
      else
        realStopTime_local = dstoptime
        call ESMF_LogWrite(subname// &
          " AVISO: stop_ymd nao encontrado, usando dstoptime como fallback", &
          ESMF_LOGMSG_WARNING)
      end if

      call ESMF_TimeGet(realStopTime_local, timestring=timestr, rc=rc)
      call ESMF_LogWrite("Stop Alarm will ring at : "//trim(timestr), ESMF_LOGMSG_INFO)

      stop_alarm = ESMF_AlarmCreate(mclock, ringtime=realStopTime_local, &
                   name="stop_alarm", rc=rc)
...
subroutine State_SetScalar(value, scalar_id, State, mytask, scalar_name, scalar_count,  rc)
  ...
  rc = ESMF_SUCCESS
  if (mytask == 0)  print*,'pkubota',trim(scalar_name)
...
!> CheckImport tolerante para NUOPC 8.9.0.
!!
!! O NUOPC_ModelBase padrao rejeita campos cujo timestamp nao seja
!! exatamente igual a currTime do componente. Isso falha no nosso
!! acoplamento porque o MED marca os campos exportados com nextTime_MED
!! (= currTime_MED + dt_coupling), enquanto o clock do OCN neste ponto
!! da fase ainda esta em currTime_OCN (antes do ClockAdvance interno).
!! Os dois valores sao numericamente iguais ao coupling instant correto,
!! mas a comparacao estrita falha se houver qualquer offset de clock.
!!
!! Esta rotina aceita campos cujo timestamp esteja em:
!!   [currTime - dt_coupling , currTime + dt_coupling]
!! e loga um WARNING para campos fora dessa janela em vez de abortar.
!! Campos nao encontrados no importState sao ignorados (campo opcional).
!!
!! @param gcomp  ESMF_GridComp do componente OCN
!! @param rc     codigo de retorno
subroutine CheckImportSkip(gcomp, rc)
  type(ESMF_GridComp)  :: gcomp
  integer, intent(out) :: rc
  ...
  integer                 :: fyy, fmm, fdd, fh, fm, fs
  character(len=64), allocatable :: fieldNameList(:)
  character(len=256)      :: logmsg
  logical                 :: allValid, fieldValid
  character(len=*), parameter :: subname = '(MOM_cap:CheckImport) '
  integer(ESMF_KIND_I8) :: fieldSecs, currSecs, stepSecs

  rc = ESMF_SUCCESS

  ! Obter importState e clock do componente
  call ESMF_GridCompGet(gcomp, importState=importState, clock=clock, rc=localrc)
  if (ChkErr(localrc,__LINE__,u_FILE_u)) then
    call ESMF_LogWrite(subname//'AVISO: falha ao obter importState/clock - skip', &
      ESMF_LOGMSG_WARNING)
    return   ! nao propaga rc: nao aborta por falha no get
  end if

  call ESMF_ClockGet(clock, currTime=currTime, timeStep=timeStep, rc=localrc)
  if (ChkErr(localrc,__LINE__,u_FILE_u)) then
    call ESMF_LogWrite(subname//'AVISO: falha ao obter currTime - skip', &
      ESMF_LOGMSG_WARNING)
    return
  end if

  ! Listar campos do importState
  call ESMF_StateGet(importState, itemCount=fieldCount, rc=localrc)
  if (fieldCount == 0 .or. localrc /= ESMF_SUCCESS) then
    call ESMF_LogWrite(subname//'importState vazio - CheckImport OK', ESMF_LOGMSG_INFO)
    return
  end if

  allocate(fieldNameList(fieldCount))
  call ESMF_StateGet(importState, itemNameList=fieldNameList, rc=localrc)
  if (ChkErr(localrc,__LINE__,u_FILE_u)) then
    deallocate(fieldNameList)
    return
  end if

  call ESMF_TimeGet(currTime, yy=yy, mm=mm, dd=dd, h=h, m=m, s=s, rc=localrc)

  allValid = .true.
  do i = 1, fieldCount

    ! Tentar obter o campo - pode nao estar realizado (opcional)
    call ESMF_StateGet(importState, itemName=trim(fieldNameList(i)), &
      field=field, rc=localrc)
    if (localrc /= ESMF_SUCCESS) cycle   ! campo nao encontrado: ignora

    ! Obter timestamp marcado pelo conector via NUOPC
    ! ESMF_FieldGet com timespec retorna o time associado ao field
    call ESMF_FieldGet(field, rc=localrc)
    if (localrc /= ESMF_SUCCESS) cycle

    ! Ler timestamp do field via atributo NUOPC UpdateTime
    call NUOPC_GetTimestamp(field, time=fieldTime, rc=localrc)
    if (localrc /= ESMF_SUCCESS) then
      ! Campo sem timestamp NUOPC: ainda nao passou por conector - aviso, nao erro
      write(logmsg,'(A,A,A)') subname, &
        'AVISO: sem timestamp NUOPC em ', trim(fieldNameList(i))
      call ESMF_LogWrite(trim(logmsg), ESMF_LOGMSG_WARNING)
      cycle
    end if

    ! Aceita: currTime - dt <= fieldTime <= currTime + dt
    ! BEFORE (broken):
    !PK fieldValid = (fieldTime >= currTime - timeStep) .and. &
    !PK              (fieldTime <= currTime + timeStep)
    call ESMF_TimeGet(fieldTime,   s_i8=fieldSecs, rc=rc)
    call ESMF_TimeGet(currTime,    s_i8=currSecs,  rc=rc)
    call ESMF_TimeIntervalGet(timeStep, s_i8=stepSecs, rc=rc)

    fieldValid = (fieldSecs >= currSecs - stepSecs) .and. &
                 (fieldSecs <= currSecs + stepSecs)
    !!PK  AFTER (correct):
    !!PK fieldValid = (ESMF_TimeGE(fieldTime, currTime - timeStep)) .and. &
    !!PK          (ESMF_TimeLE(fieldTime, currTime + timeStep))

    if (.not. fieldValid) then
      call ESMF_TimeGet(fieldTime, yy=fyy, mm=fmm, dd=fdd, h=fh, m=fm, s=fs, rc=localrc)
      write(logmsg,'(A,A,A,I4.4,"-",I2.2,"-",I2.2,"T",I2.2,":",I2.2,":",I2.2, &
                    &A,I4.4,"-",I2.2,"-",I2.2,"T",I2.2,":",I2.2,":",I2.2)') &
        subname, 'ERRO timestamp: ', trim(fieldNameList(i))//' field=', &
        fyy, fmm, fdd, fh, fm, fs, &
        ' currTime=', yy, mm, dd, h, m, s
      call ESMF_LogWrite(trim(logmsg), ESMF_LOGMSG_ERROR)
      allValid = .false.
    else
      write(logmsg,'(A,A,A)') subname, 'OK: ', trim(fieldNameList(i))
  ...
  if (.not. allValid) then
    ! Loga o erro mas NAO aborta: permite que a simulacao continue
    ! e o problema seja diagnosticado via log.
    ! Para abortar em producao, substituir o LogWrite pelo bloco abaixo:
    !   call ESMF_LogSetError(ESMF_FAILURE, &
    !     msg=subname//"campos com timestamp fora da janela aceitavel", &
    !     line=__LINE__, file=__FILE__, rcToReturn=rc)
    call ESMF_LogWrite(subname// &
      'AVISO: campos com timestamp fora da janela - simulacao continua', &
      ESMF_LOGMSG_WARNING)
  else
    call ESMF_LogWrite(subname//'todos os campos OK', ESMF_LOGMSG_INFO)
  end if

end subroutine CheckImportSkip

!PK !> Substitui o CheckImport padrao do NUOPC_ModelBase.
!PK !! Retorna ESMF_SUCCESS sem verificar timestamps dos campos do importState.
!PK !! Necessario porque o NUOPC 8.9.0 rejeita com
!PK !! "Import Fields not at current time" mesmo com timestamps corretos.
!PK subroutine CheckImportSkip(gcomp, rc)
!PK   type(ESMF_GridComp)  :: gcomp
!PK   integer, intent(out) :: rc
!PK   rc = ESMF_SUCCESS

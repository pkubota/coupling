...
! ===================================================================
! JRA55 forcing via data_override - ocean-only without ATM component
! Inserted after IOB->fluxes conversion
! ===================================================================
! Temporary arrays for JRA atmospheric state
  real, dimension(G%isc:G%iec, G%jsc:G%jec) :: &
    uas_jra, vas_jra, rsds_jra, rlds_jra, &
    prra_jra, prsn_jra, wind_speed, cd
  logical :: overrode
  ...
  ! ===================================================================
  ! JRA55 forcing via data_override - ocean-only without ATM component
  ! Called AFTER the main IOB->fluxes loop so i,j are free to reuse.
  ! Overrides radiation, precipitation, and LW from JRA55 files.
  ! Wind stress is handled in convert_IOB_to_forces (separate subroutine).
  ! ===================================================================
  call data_override('OCN', 'uas_jra',   uas_jra,  Time, override=overrode)
  call data_override('OCN', 'vas_jra',   vas_jra,  Time, override=overrode)
  call data_override('OCN', 'rsds_jra',  rsds_jra, Time, override=overrode)
  call data_override('OCN', 'rlds_jra',  rlds_jra, Time, override=overrode)
  call data_override('OCN', 'lprec_jra', prra_jra, Time, override=overrode)
  call data_override('OCN', 'fprec_jra', prsn_jra, Time, override=overrode)

  if (overrode) then
    do j = js, je ; do i = is, ie
      wind_speed(i,j) = sqrt(uas_jra(i,j)**2 + vas_jra(i,j)**2) + 1.0e-10
      ! Radiation by band (positive down = heating ocean)
      fluxes%sw_vis_dir(i,j) = US%W_m2_to_QRZ_T * 0.285 * rsds_jra(i,j) * G%mask2dT(i,j)
      fluxes%sw_vis_dif(i,j) = US%W_m2_to_QRZ_T * 0.285 * rsds_jra(i,j) * G%mask2dT(i,j)
      fluxes%sw_nir_dir(i,j) = US%W_m2_to_QRZ_T * 0.215 * rsds_jra(i,j) * G%mask2dT(i,j)
      fluxes%sw_nir_dif(i,j) = US%W_m2_to_QRZ_T * 0.215 * rsds_jra(i,j) * G%mask2dT(i,j)
      fluxes%sw(i,j)         = fluxes%sw_vis_dir(i,j) + fluxes%sw_vis_dif(i,j) + &
                               fluxes%sw_nir_dir(i,j) + fluxes%sw_nir_dif(i,j)
      fluxes%lw(i,j)         = US%W_m2_to_QRZ_T * rlds_jra(i,j) * G%mask2dT(i,j)
      fluxes%lprec(i,j)      = kg_m2_s_conversion * prra_jra(i,j) * G%mask2dT(i,j)
      fluxes%fprec(i,j)      = kg_m2_s_conversion * prsn_jra(i,j) * G%mask2dT(i,j)
    enddo ; enddo
  endif
...
subroutine convert_IOB_to_forces(IOB, forces, index_bounds, Time, G, US, CS)
  ...
  ! JRA55 wind stress via bulk formula
  real, dimension(SZI_(G),SZJ_(G)) :: uas_jra_f !< JRA55 zonal wind [m s-1]
  real, dimension(SZI_(G),SZJ_(G)) :: vas_jra_f !< JRA55 meridional wind [m s-1]
  real :: wspd_jra      !< JRA55 wind speed magnitude [m s-1]
  logical :: overrode_f !< flag: data_override provided JRA wind
...
!###  call user_alter_mech_forcing(forces, Time, G, CS%urf_CS)
  ...
  ! ===================================================================
  ! JRA55 wind stress via bulk formula - ocean-only without ATM component
  ! Uses the same uas_jra/vas_jra fields as convert_IOB_to_fluxes.
  ! Cd_neut = 1.3e-3 (NCAR bulk, Large & Yeager 2009)
  ! Stress units: [R Z L T-2 ~> Pa] via Pa_conversion
  ! ===================================================================
  call data_override('OCN', 'uas_jra', uas_jra_f, Time, override=overrode_f)
  call data_override('OCN', 'vas_jra', vas_jra_f, Time, override=overrode_f)
  if (overrode_f) then
    do j = js, je ; do i = is, ie
      wspd_jra = sqrt(uas_jra_f(i,j)**2 + vas_jra_f(i,j)**2) + 1.0e-10
      forces%taux(i,j) = Pa_conversion * 1.225 * 1.3e-3 * wspd_jra * uas_jra_f(i,j) * G%mask2dT(i,j)
      forces%tauy(i,j) = Pa_conversion * 1.225 * 1.3e-3 * wspd_jra * vas_jra_f(i,j) * G%mask2dT(i,j)
    enddo ; enddo
    call pass_vector(forces%taux, forces%tauy, G%Domain, halo=1)
  endif
